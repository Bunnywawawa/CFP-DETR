...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'BasicBlock_PConv', 'BottleNeck_PConv', 'EL_CFPDETR', 'BasicBlock_AdaptivePConv_OriginalStyle' , 'BottleNeck_PConv_Rep', 'BasicBlock_Faster_Block', 'BottleNeck_Faster_Block',
           ...
           'ContextGuideFusionModule', 'C3_DEConv', 'C2f_DEConv', 'BasicBlock_DEConv', 'BottleNeck_DEConv'
           ]
...
class AdaptivePartial_conv3_Rep(nn.Module):
    def __init__(self, dim, ratio=1.0, forward='split_cat'):
        super().__init__()
        self.dim_conv3 = max(1, int(dim * ratio))  # 确保至少有一个通道
        self.dim_untouched = dim - self.dim_conv3
        self.forward_method = forward
        
        # 只对部分通道进行卷积
        self.partial_conv = nn.Conv2d(
            self.dim_conv3,  # 输入通道数按比例减少
            self.dim_conv3,  # 输出通道数也按比例减少
            3, 1, 1, bias=False
        )
    ...
    def forward(self, x):
        ...
        elif self.forward_method == 'slicing':
            return self.forward_slicing(x)
        else:
            raise ValueError(f"Unsupported forward method: {self.forward_method}")


class BasicBlock_AdaptivePConv_OriginalStyle(BasicBlock):
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d', ratio=1.0, **kwargs):
        # 确保比例在0-1之间
        ...
        # 替换 branch2b 为我们的自适应部分卷积
        self.branch2b = nn.Sequential(
            AdaptivePartial_conv3_Rep(dim=ch_out, ratio=ratio),
            nn.BatchNorm2d(num_features=ch_out),
            nn.ReLU()
        )
