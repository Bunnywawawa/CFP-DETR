...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 ConvNormLayer,RFAConv, RFCAConv, RFCBAMConv):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3):
                ...
                n = 1
        elif m in (AIFI, AIFI_LPE, TransformerEncoderLayer_LocalWindowAttention, TransformerEncoderLayer_DAttention, TransformerEncoderLayer_HiLo,GCDM):
            c2 = ch[f]
            args = [ch[f], *args]
        ...
        # --------------ASF--------------
            ...
            # block_type lookup (support extra_modules via ultralytics.nn.extra_modules import *)
            _bt = args[1]
            block_type = globals().get(_bt)
            if block_type is None:
                import ultralytics.nn.extra_modules as _extra
                block_type = getattr(_extra, _bt, None)
            if block_type is None:
                raise KeyError(f"Block type '{_bt}' not found. Ensure it's imported into ultralytics.nn.extra_modules and accessible.")
